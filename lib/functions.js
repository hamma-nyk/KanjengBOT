...
const wait = async (media) => new Promise(async (resolve, reject) => {
    ...
    try {
    	...
    	resolve({video: await getBuffer(`https://media.trace.moe/video/${anilist_id}/${encodeURIComponent(filename)}?t=${at}&token=${tokenthumb}`), teks: `╭────────────────────────\n├ KANJENG BOT HUNTING ANIME\n╰────────────────────────\n [+] Title Jepang : *${title}*\n [+] Title Romaji : *${title_romaji}*\n [+] Title Inggris : *${title_english}*\n [+] Episode : *${episode}*\n [+] Ecchi : *${ecch()}*\n [+] Season  : *${season}*\n ${belief()}\n`});
	} catch (e) {
		console.log(e)
